<template>
  <div class="app-container">
    ...
    <div class="statistics-layout">
      ...
      <div style="padding: 10px;border-left:1px solid #DCDFE6">
        <el-select v-model="monthPic" style="float: right;z-index: 1"
                   placeholder="请选择类型" @change="selectMonthData">
          <el-option
            v-for="item in selectPayLists"
            :key="item"
            :label="item"
        ...
        <el-col :span="20">
          <div style="padding: 10px;border-left:1px solid #DCDFE6">
            <div>
              ...
              :monthPic="monthPic"></ve-histogram>
            </div>
          </div>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    selectMonthData() {
      ...
      setTimeout(() => {
        ...
        /** 第三个饼状图 **/
        getPayAndRevenue(this.monthPic).then(response => {
          console.log("来了")
          for (let i = 0; i < response.data.length; i++) {
            let item = response.data[i];
