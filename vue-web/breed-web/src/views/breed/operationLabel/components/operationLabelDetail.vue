  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="label"
      ...
      <el-form-item v-show="false" label="id：" prop="label" isEdit>
        <el-input v-model.trim="label.olid"></el-input>
      </el-form-item>
      <el-form-item label="标签内容：" prop="label" isEdit>
        <el-input v-model.trim="label.labelContent"></el-input>
      </el-form-item>
      <el-form-item>
...
<script>
import {getOperationLabelInfo, fetchList, updateOperationLabel, createOperationLabel} from '@/api/operationLabel';
...
export default {
  ...
  created() {
    if (this.isEdit) {
      getOperationLabelInfo(this.$route.query.id).then(response => {
        this.label = response.data;
      });
    } else {
      this.label = Object.assign({}, defaultlabel);
    }
    this.getSelectLabelList();
  },
  methods: {
    getSelectLabelList() {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm('是否提交数据', '提示', {
            ...
            if (this.isEdit) {
              updateOperationLabel(this.label).then(response => {
                this.$message({
                  message: '修改成功',
                  type: 'success',
                  duration: 1000
              ...
              createOperationLabel(this.label).then(response => {
