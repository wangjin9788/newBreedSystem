    ...
    <div class="table-container">
      <el-table ref="revenueTable"
        ...
        <el-table-column label="编号" width="100" align="center" >
          <template slot-scope="scope">{{ scope.row.olId }}</template>
        </el-table-column>
        <el-table-column label="标签内容" align="center">
          <template slot-scope="scope">{{ scope.row.labelContent }}</template>
        </el-table-column>
...
<script>
import {fetchList, deleteOperationLabel} from '@/api/operationLabel';
...
export default {
  ...
  methods: {
    ...
    handleUpdate(index, row) {
      this.$router.push({path: '/feed/updateFerment', query: {id: row.olId}});
    },
    ...
    handleDelete(index, row) {
      this.$confirm('是否要删除该养殖记录?', '提示', {
        ...
        deleteOperationLabel(row.olId).then(response => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
